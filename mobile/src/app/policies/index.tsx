...
import { IPolicy } from '@/src/models/policy.interface'
import { useDeletePolicyMutation, useGetPoliciesQuery } from '@/src/redux/apis/policy.api'
...
const Policies = () => {
    ...
    const { data, isLoading, refetch } = useGetPoliciesQuery({
        page: page + 1,
        limit: itemsPerPage,
                ...
                <ScrollView horizontal refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} />}>
                    <DataTable style={{ backgroundColor: theme.colors.cardBg }}>
                        <DataTable.Header>
                            ...
                            <DataTable.Title
                                sortDirection={sortConfig?.key === "name.name" ? sortConfig.direction : undefined}
                                onPress={() => handleSort("name.name")}
                            ...
                            <DataTable.Title
                                sortDirection={sortConfig?.key === "provider" ? sortConfig.direction : undefined}
                                onPress={() => handleSort("provider")}
                                style={{ width: 180, marginRight: 20 }}>
                                Provider
                            </DataTable.Title>
                            <DataTable.Title
                                sortDirection={sortConfig?.key === "expiryDate" ? sortConfig.direction : undefined}
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => {
                                ...
                                return <DataTable.Row >
                                    ...
                                    <DataTable.Cell style={{ width: 180, marginRight: 20 }}>
                                        <Text numberOfLines={2}>{item.provider}</Text>
                                    </DataTable.Cell>
